...
describe("App", () => {
  ...
  it('test product', async () => {
    ...
    const productTabProcess = Windows.getTabProcessesById(PRODUCT_WINDOW_ID, TAB_PRODUCT_ID)
  ...
  it('get product', async () => {
    ...
    const productList = await OBRest.getInstance().createCriteria(currentEntity.entityName)
    ...
    let origProduct = productList.find(product => product.id === PRODUCT_ID) as OBProduct
    ...
    const product = await RecordService.onSave({ ...origProduct }) as OBProduct
    ...
    expect(errMsg).toEqual("Error: The record you are saving has already been changed by another user or process. Cancel your changes and refresh the data by clicking the refresh button.")
