...
describe("App", () => {
  ...
  it('test behav', async () => {
    ...
    const headerTab = Windows.getTabById(PHYSICAL_INVENTORY_WINDOW_ID, TAB_HEADER_ID);
    expect(headerTab).not.toBeUndefined()

    expect(headerTab.uIPattern).toBe(UI_PATTERNS.STD)

    const accountingTab = Windows.getTabById(PHYSICAL_INVENTORY_WINDOW_ID, TAB_ACCOUNTING_ID);
